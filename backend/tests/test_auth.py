...
async def test_login_wrong_password(client: AsyncClient, test_user_data):
    ...
    login_data = {"username": test_user_data["email"], "password": "WrongPassword123!"}
    response = await client.post("/api/v1/auth/login", data=login_data)
...
async def test_refresh_token_invalid(client: AsyncClient):
    ...
    response = await client.post(
        "/api/v1/auth/refresh", params={"refresh_token": "invalid_refresh_token"}
    )
...
async def test_inactive_user_cannot_login(
    client: AsyncClient, test_user_data, db_session: AsyncSession
):
