...
class Appointment(BaseModel):
    ...
    appointment_type: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="ROUTINE | URGENT | EMERGENCY | FOLLOWUP | etc."
    )

    # Service category
    service_category: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="General Practice | Specialist | Therapy | etc."
    )
    ...
    end_datetime: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="When appointment is scheduled to end"
    )

    # Duration in minutes
    duration_minutes: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="Expected duration (optional if end_datetime provided)"
    )
