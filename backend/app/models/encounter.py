...
class Encounter(BaseModel):
    ...
    period_start: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True, comment="Start time of encounter"
    )

    period_end: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="End time of encounter"
    )

    # Reason for encounter (coded)
    reason_code: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="SNOMED CT or ICD-10 code for reason"
    )

    # Reason for encounter (human-readable)
    reason_display: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Human-readable reason (e.g., 'Annual physical', 'Chest pain')"
    )

    # Additional notes
    notes: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Additional notes about the encounter"
    )
    ...
    observations: Mapped[list["Observation"]] = relationship(
        "Observation", back_populates="encounter", cascade="all, delete-orphan"
    )
