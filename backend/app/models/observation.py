...
class Observation(BaseModel):
    ...
    value_quantity: Mapped[Optional[float]] = mapped_column(
        Numeric(10, 2),
        nullable=True,
        comment="Numerical value (e.g., 120 for systolic BP)"
    )
    ...
    value_unit: Mapped[Optional[str]] = mapped_column(
        String(50),
        nullable=True,
        comment="Unit (e.g., 'mmHg', 'kg', 'celsius')"
    )
    ...
    value_string: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Text value (e.g., 'Positive', 'Negative', 'Moderate')"
    )
    ...
    effective_datetime: Mapped[Optional[datetime]] = mapped_column(
        ...
        nullable=True,
        index=True,
        comment="When observation was taken"
    )
    ...
    issued: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        comment="When observation result was released"
    )
    ...
    interpretation: Mapped[Optional[str]] = mapped_column(
        String(50),
        nullable=True,
        comment="normal | high | low | critical | etc."
    )
    ...
    encounter: Mapped[Optional["Encounter"]] = relationship("Encounter", back_populates="observations")
