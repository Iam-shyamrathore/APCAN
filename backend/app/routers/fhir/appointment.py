...
from app.schemas.fhir.appointment import AppointmentCreate, AppointmentUpdate, AppointmentResponse, FHIRAppointment
...
async def search_appointments(
    ...
    if date:
        try:
            ...
            query = query.where(
                ...
                Appointment.start_datetime < datetime(date_obj.year, date_obj.month, date_obj.day + 1)
            )
        except ValueError:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if date_le:
        try:
            ...
            # Include entire day
            date_le_end = datetime(date_le_obj.year, date_le_obj.month, date_le_obj.day) + timedelta(days=1)
