...
from app.schemas.fhir.observation import ObservationCreate, ObservationUpdate, ObservationResponse, FHIRObservation
...
async def search_observations(
    patient: Optional[str] = Query(None, description="Patient reference (e.g., 'Patient/123' or '123')"),
    encounter: Optional[str] = Query(None, description="Encounter reference (e.g., 'Encounter/456' or '456')"),
    code: Optional[str] = Query(None, description="LOINC code (e.g., '8480-6' for systolic BP)"),
    category: Optional[str] = Query(None, description="Observation category (vital-signs, laboratory, etc.)"),
    ...
    """
    query = select(Observation).where(Observation.is_deleted == False)
    ...
    if date:
        ...
        try:
            ...
            query = query.where(
                ...
                Observation.effective_datetime < datetime(date_obj.year, date_obj.month, date_obj.day + 1)
            )
        except ValueError:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
