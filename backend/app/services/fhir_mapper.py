...
class FHIRMapper:
    ...
    def encounter_to_fhir(encounter: Encounter, include_patient: bool = True) -> FHIREncounter:
        ...
        """
        # Meta information
        meta = Meta(versionId=str(encounter.id), lastUpdated=encounter.updated_at)
        ...
        subject = Reference(
            reference=f"Patient/{encounter.patient_id}", display=patient_display, type="Patient"
        )
        ...
        class_code, class_display = encounter_class_map.get(
            encounter.encounter_class.lower(), ("AMB", "ambulatory")
        )
        ...
        if encounter.period_start or encounter.period_end:
            period = Period(start=encounter.period_start, end=encounter.period_end)
    ...
    def appointment_to_fhir(
        appointment: Appointment, include_patient: bool = True
    ) -> FHIRAppointment:
        """
        ...
        """
        # Meta information
        meta = Meta(versionId=str(appointment.id), lastUpdated=appointment.updated_at)
        ...
        participants.append(
            {
                ...
                "required": "required",
                "status": (
                    "accepted" if appointment.status in ["booked", "fulfilled"] else "tentative"
                ),
            }
        )
    ...
    def observation_to_fhir(
        observation: Observation, include_patient: bool = True
    ) -> FHIRObservation:
        """
        ...
        """
        # Meta information
        meta = Meta(versionId=str(observation.id), lastUpdated=observation.updated_at)
        ...
        subject = Reference(
            reference=f"Patient/{observation.patient_id}", display=patient_display, type="Patient"
        )
        ...
        if observation.encounter_id:
            encounter_ref = Reference(
                reference=f"Encounter/{observation.encounter_id}", type="Encounter"
            )
        ...
        reference_range = []
        if (
            observation.reference_range_low is not None
            or observation.reference_range_high is not None
        ):
            range_dict = {}
        ...
        if observation.notes:
            note.append({"text": observation.notes})
    ...
    def patient_to_fhir(patient: Patient) -> FHIRPatient:
        ...
        """
        # Meta information
        meta = Meta(versionId=str(patient.id), lastUpdated=patient.updated_at)
        ...
        if patient.emergency_contact_name and patient.emergency_contact_phone:
            ...
            contacts.append(
                PatientContact(
                    ...
                    telecom=[ContactPoint(system="phone", value=patient.emergency_contact_phone)],
                )
            )
