...
async def create_test_observations(
    session: AsyncSession, encounters: List[Encounter]
) -> List[Observation]:
...
async def seed_mock_data():
    ...
    async for session in get_async_session():
        try:
            ...
            encounters = await create_test_encounters(
                session, patients, providers, count_per_patient=3
            )
            appointments = await create_test_appointments(
                session, patients, providers, count_per_patient=2
            )
