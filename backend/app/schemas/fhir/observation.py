...
class FHIRObservation(BaseModel):
    ...
    category: Optional[List[CodeableConcept]] = Field(
        None, description="Classification of type of observation"
    )
    ...
    encounter: Optional[Reference] = Field(
        None, description="Healthcare event during which observation was made"
    )

    effectiveDateTime: Optional[datetime] = Field(
        None, description="Clinically relevant time/time-period for observation"
    )
    issued: Optional[datetime] = Field(
        None, description="Date/Time this version was made available"
    )
    ...
    interpretation: Optional[List[CodeableConcept]] = Field(
        None, description="High, low, normal, etc"
    )
    note: Optional[List[dict]] = Field(None, description="Comments about the observation")

    referenceRange: Optional[List[dict]] = Field(
        None, description="Reference range for observations"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "referenceRange": [
                    {
                        "low": {"value": 90.0, "unit": "mmHg"},
                        "high": {"value": 120.0, "unit": "mmHg"},
                    }
                ],
            }
        }
